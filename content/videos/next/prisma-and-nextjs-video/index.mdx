...
<Tabs>
  <Tab name="JS">
    {/* prettier-ignore */}
    ```sh npx create-next-app ```
  </Tab>
  <Tab name="TS">
    {/* prettier-ignore */}
    ```sh npx create-next-app --ts ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab name="JS">
    {/* prettier-ignore */}
    ```sh yarn add -D prisma npx prisma init ```
  </Tab>
  <Tab name="TS">
    {/* prettier-ignore */}
    ```sh yarn add prisma typescript ts-node @types/node --dev npx prisma init ```
  </Tab>
</Tabs>
...
<Note>
  You **never** need to run this command if you're using MongoDB. MongoDB doesn't require you to setup a schema. So you can ignore anything to do with migrations if you're using MongoDB.
</Note>
...
<Image src="/images/db-client-ts.png" width={502 / 2} height={374 / 2} />
